...
country_labels <- full_country |> 
  #Our graph is a snapshot of countries in 2007, but the dataset includes many more years. So we need to filter the data first to only 2017.
  filter(year == 2007,
...
full_country |> 
  ...
  geom_text_repel(data = year_label_rw, family = "Garamond", 
                  size = 6, color = "#555555") +  
  ...
  annotate(geom = "text", 
           ...
           y = famine$lifeExp,
           label = "Famine, 1944", family = "Garamond", 
           size = 6, color = "#00A1DE",
           vjust = 1.2) +
  annotate(geom = "rect", 
           ymin = 42, ymax = 55,
