...
A basic way to create graphs in R is with geom_polygon, as discussed in your first reading assignment.

```{r}
#R package corresponding to the R for Data Viz book
library(socviz)

#R's maps package for pre-drawn map data
library(maps)
library(mapproj)
#ggthemes gives us some extra themes for graphs, including theme_map
library(ggthemes)

#grab the data on US states from the maps package
us_states <- map_data("state")
head(us_states)

#let's create a quick map of the US using geom_polygon!
#coord_map function comes from mapproj package
...
```
 ...
 We will recreate some of the maps from the 2016 election. Remember these are basic choropleth maps.
```{r}
#election data is part of the socviz package, so we can use it directly. 
...
#When we have map data, we often need to combine map data with our own dataset so have all the location and response variables together. Here we join the us_states data with election data
election$region <- tolower(election$state)
us_states_elec <- left_join(us_states, election)

#Now we can plot the election data on a map
...
#Let's clean this up and use party colors plus a white background
ggplot(data = us_states_elec,
             mapping = aes(x = long, y = lat,
                           group = group, fill = party)) +
  geom_polygon(color = "gray90", size = 0.1) +
...
#lastly, let's look at winning margins scaled by color, where we exclude DC because it is a blue outlier  
ggplot(data = subset(us_states_elec,
...
Include in this projects is a folder called "shapefiles" which includes  
...
The last column is the magical `geometry` column with the latitude/longitude details for the borders for every country. RStudio only shows you 50 columns at a time in the RStudio viewer, so you’ll need to move to the next page of columns with the » button in the top left corner.
...
Complete the code below. Here is a nice source for choosing colors, including hex codes:
[RChartColorPalettes](https://r-charts.com/color-palettes/)
...
How do we find the common names of projections? Remember from your reading, 
you can use the following notation, with the full list of projections [here](https://proj.org/operations/projections/index.html):
...
```{r}
csumb_counties <- ca_counties |> 
  filter(NAME %in% c("Monterey", "Santa Cruz", "San Benito"))


ggplot() +
  geom_sf(data = only_ca, color = "#112E51", linewidth = 1.5, fill = "white") +
  geom_sf(data = csumb_counties, fill = "#125A37", color = "#DDDAC7") +
  theme_void()
```
...
#WARNING, this code will take a while! 
