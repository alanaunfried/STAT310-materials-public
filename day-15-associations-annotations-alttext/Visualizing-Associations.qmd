...
library(ggtext) #used in regression chunk to change title colors
library(ggcorrplot) #for correlograms
library(GGally)#for faceted graph of distributions + scatterplots, etc
...
```{r}
#an example of saving a plot as an R object...
peng_plot <- pengs |> 
  drop_na(sex) |> 
...
This is a good opportunity to try some basic annotations! Let's try text first.

```{r}
#start with the plot from the last code chunk and add on to it

peng_plot +
  #add a text annotation! specify where it should start and what it should say 
  #by default these center justify (center of word is at coordinate) but I want to left justify
  annotate(geom = "text", x=52, y=15, label = "Gentoo", color = "#FDE725FF", hjust="left", fontface="bold", size = 6) +
  annotate(geom = "text", x=30, y=15, label = "Adelie", color = "#440154FF", hjust="left", fontface="bold", size = 6) +
  annotate(geom = "text", x=53, y=19, label = "Chinstrap", color = "#21908CFF", hjust="left", fontface="bold", size = 6) +
  #since I added the annotations, I do not need the legend for color
  guides(color = "none")

#this is how I found the color codes I wanted to use
viridis::viridis(3)
# "#440154FF" "#21908CFF" "#FDE725FF"
```

The previous figure might be getting to crowded, so let's consider splitting it by facets. Now I might consider coloring by sex and not species since the species are split into separate figures.
...
```{r}
#| label: regression and title color

#remember, need ggtext package for this code to work

#note we have missing data on the sex variable. We will filter out the missing cases so we don't get regression lines for the "missing" sex category
unique(pengs$sex)
...
pengs |> 
  ...
  labs(x = "Culmen (Bill) Length (mm)",
       ...
       #adding colors to Male and Female words in subtitle via ggtext package, so we don't need a legend
  ...
  #free_x allows each Species to have a custom range on the x axis
  facet_wrap(~species, scales = "free_x") +
  #no legends for shape or color due to legend embedded in subtitle
  guides(shape = "none", color = "none") +
  #geom_smooth adds the regression lines. The aes for color creates a different line for each sex; if you remove this aes, you get one line for each species. LM method is Linear Model. se = False means it will not show standard error bars
...
```{r}
#remember need to load ggcorplot package for this code to work!
...
#need library(GGally) for this code to work
...
Sometimes we want to show a comparison between two values that are paired or connected in some way.
